...
- [ ] **`called_time` が付かないと待機料が1円も出ない**（2026-07-31 発見）。
      `convertWaitLogsToTimeline` の荷待ち時間は「到着〜呼出(`called_time`)」で算出するが、
      `called_time` を設定するのは `advance_wait_status` RPC のみで、これを呼ぶのは
      `AdminDashboard.tsx` の呼出ボタンだけ。**荷主側が管理ダッシュボードを操作しない現場では
      `called_time` が永久にNULLのままで、待機イベント自体が生成されず待機料がゼロになる。**
      ドライバー側だけで運用した場合に請求根拠が一切残らないため、優先度は高い。
      対策候補: 到着〜作業完了から荷役時間を差し引く、ドライバー側に「呼ばれた」打刻を持たせる等
