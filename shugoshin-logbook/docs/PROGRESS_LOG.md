...
- **圏外での荷役開始が記録できない**。`pending_punches.punch_type` は
  `'arrival'` / `'completion'` のみを許可しており、荷役開始の圏外申請ができない。
  荷役開始が課金の境界になった以上、圏外だとこの待機の待機料を失う。
  CHECK制約の拡張と `approve_pending_punch` の対応が必要
