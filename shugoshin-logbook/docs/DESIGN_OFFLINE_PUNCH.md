...
| # | 論点 | 選択肢 | 備考 |
|---|---|---|---|
| **①** | 等級Cの `arrival_time` に何を入れるか | (a) `claimed_at` を入れる（待機時間が実態に合う。ただし `trg_force_wait_log_arrival` に承認RPC用の例外が要る）<br>(b) サーバー時刻のまま入れ、`claimed_at` は別カラムでのみ保持し、待機時間の算定は `claimed_at` を使う | **(b) を推す。** 時刻強制上書きトリガーに例外を開けると、そこが将来の偽装経路になる。トリガーは無傷のまま、算定側が `COALESCE(claimed_at, arrival_time)` を見る形が安全 |
| **②** | 承認の期限 | 無期限 / 30日 / 締め日まで | 待機料は請求サイクルに乗るため、締め後の承認をどう扱うかは業務側の判断 |
| **③** | 自己承認の禁止を貫くか | 一人親方・小規模事業者では管理者＝ドライバーになりうる | 禁止を貫くと小規模事業者が使えない。「自己承認の場合はその旨を記録に残す」妥協案も検討に値する |
| **④** | 却下された申請をドライバーが再申請できるか | 可 / 不可 | 可とする場合、却下→再申請の履歴が残ることを明示 |
| **⑤** | iOS PWA でのバックグラウンド送信 | Service Worker の Background Sync は iOS Safari 非対応 | 「次にアプリを開いたとき送信」で妥協するのが現実的。運用懸念③（作業完了の押し忘れ通知）と同じ制約に当たる |
